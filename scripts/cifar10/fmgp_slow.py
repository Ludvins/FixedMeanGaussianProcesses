...
# Test the model
test_start = time()
test_metrics = score(ue, test_loader, SoftmaxClassification, use_tqdm=True)
test_end = time()

test_metrics["method"] = "fmgp slow"
...
for angle in angles:
    ...
    metrics = score(ue, test_loader, SoftmaxClassification, use_tqdm=False)
...
print(df)
df.to_csv(
    path_or_buf=f"results/cifar10/fmgp_slow_{args.resnet}_{args.seed}.csv",
    encoding="utf-8",
    index=False,
)
...
np.savetxt(
    f"results/cifar10_ood/fmgp_{args.resnet}_{args.seed}.txt",
    predictions_ood,
)
