...
# Train the model
train_start = time()
losses = mfvi.fit(train_loader=train_loader, iterations=8000, verbose=True)
train_end = time()

# Test the model
test_start = time()
test_metrics = score_samples(mfvi, test_loader, SoftmaxClassificationSamples, verbose=True)
test_end = time()

test_metrics["method"] = "mfvi"
...
df.to_csv(
    path_or_buf=f"results/cifar10/mfvi_{args.resnet}_{args.seed}.csv",
    encoding="utf-8",
    index=False,
)
...
np.savetxt(
    f"results/cifar10_ood/mfvi_{args.resnet}_{args.seed}.txt",
    predictions_ood,
)
